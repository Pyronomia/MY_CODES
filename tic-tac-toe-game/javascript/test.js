...
function botLogic() {
  if (difficulty === "hard") {
    let minimaxVar = minimax(botText)["box"];
    // console.log(minimax(botText));
    ...
    // let index = activeBoxes.indexOf(minimax(botText)["box"]);
    // activeBoxes.splice(index, 1);
    activePlayer = "player";
    for (let i = 0; i < allBoxes.length; i++) {
      allBoxes[i].style.pointerEvents = "auto";
...
function minimax(player) {
  ...
  for (let i = 0; i < availSpots.length; i++) {
    ...
    if (player == botText) {
      let result = minimax(playerText);
      move.score = result["score"];
    } else {
      let result = minimax(botText);
      move.score = result["score"];
    }
